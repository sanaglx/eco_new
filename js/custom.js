...
$('.modal-open').click(function(){
	$(/*'.modal-form'*/'#quastion').css({"display":"block"});
	//$("body").addClass("fixed");
	$('.teni').show();
...
$('.arhead li').click(function(){
	var id = $(this).attr('id');
	$(this).parents('.authregarea').find('.artab').removeClass('act');
	$('.arhead li').removeClass('act');
	$(this).addClass('act');
	$("#"+id+"_b").addClass('act');
});

function hederHeight(sel,h=0){
	if ($(sel).length && !$(sel).is(":hidden")){
		let a = $(sel).offset().top
		console.log(a + "|" + sel)
		$('.page-top1').css({'height':a+h+'px'})
	}
}
